...
#$(TENSORFLOW_SOURCES_PATH)/micro/debug_log.cc \
...
CXXFLAGS = $(CFLAGS) -std=c++11 -std=gnu++11 \
...
# List of objects
#######################################
OBJECTS := $(addprefix $(BUILD_DIR)/,$(notdir $(patsubst %.cc,%.o,$(filter %.cc , $(SOURCES)))))
OBJECTS += $(addprefix $(BUILD_DIR)/,$(notdir $(patsubst %.c,%.o,$(filter %.c , $(SOURCES)))))
OBJECTS += $(addprefix $(BUILD_DIR)/,$(notdir $(patsubst %.s,%.o,$(filter %.s , $(SOURCES)))))
